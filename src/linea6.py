...
while cap.isOpened():
    ...
    if confirm and not second_confirm:
        ...
        # Calcular el ángulo entre la línea ajustada y la línea vertical
        ...
        magnitude_aguja = np.linalg.norm(vector_aguja)
        magnitude_vertical = np.linalg.norm(vector_vertical)
        angle_rad = np.arccos(dot_product / (magnitude_aguja * magnitude_vertical))
        ...
        cv2.putText(mascara_aguja_inv, f'Angulo: {angle_deg:.2f}', (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255), 2, cv2.LINE_AA)

        # Mostrar las líneas ajustadas y el ángulo
        frame_with_shapes = cv2.cvtColor(mascara_aguja_inv, cv2.COLOR_GRAY2BGR)
    else:
