...
radio = 50  # Radio inicial de la circunferencia
...
while cap.isOpened():
    ...
    if confirm and not second_confirm:
        ...
        vector_vertical = np.array([0, -1])  # Vector vertical hacia abajo
        dot_product = np.dot(vector_aguja, vector_vertical)
        norm_aguja = np.linalg.norm(vector_aguja)
        norm_vertical = np.linalg.norm(vector_vertical)
        cos_angle = dot_product / (norm_aguja * norm_vertical)
        angle = np.arccos(cos_angle) * 180 / np.pi

        # Ajustar el ángulo según la orientación
        if vx > 0:
            angle = 360 - angle

        # Mostrar el ángulo en la imagen
        cv2.putText(mascara_aguja_inv, f'Angle: {angle:.2f} deg', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255), 2, cv2.LINE_AA)
        frame_with_shapes = cv2.cvtColor(mascara_aguja_inv, cv2.COLOR_GRAY2BGR)

    # Dibujar las líneas y la circunferencia en el frame original
    if not confirm or not second_confirm:
        frame_with_shapes = cv2.cvtColor(gris, cv2.COLOR_GRAY2BGR)
        cv2.line(frame_with_shapes, (0, line_x_pixel), (width, line_x_pixel), (255, 255, 255), 2)  # Línea horizontal
        cv2.line(frame_with_shapes, (line_y_pixel, 0), (line_y_pixel, height), (255, 255, 255), 2)  # Línea vertical
        cv2.circle(frame_with_shapes, (line_y_pixel, line_x_pixel), radio, (0, 255, 0), 2)  # Circunferencia

    # Mostrar el cuadro con las líneas y la circunferencia
    cv2.imshow('PDI41', frame_with_shapes)

    # Pausar y reanudar el video
    ...
    if key == ord('e'):
        if not confirm:
            ...
            second_confirm = True
        elif not third_confirm:
            third_confirm = True
        elif not fourth_confirm:
            fourth_confirm = True
    else:
        update_lines(key)
